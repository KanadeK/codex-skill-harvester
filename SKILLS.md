...
本目录列出 v0.2.0 候选版本中的全部 17 个稳定能力。面向用户的名称可以演进，但能力 ID、触发边界和合并历史由能力目录持续维护。所有生活类 Skill 都是交互式指导：人负责观察、判断现场条件并完成物理动作。
...
“candidate”表示仓库内容和验证已为 v0.2.0 准备，但不代表远端 Release 已经存在。正式发布还要求 main 上的最终 CI、注释 tag、Release 资产、校验和、隔离安装/调用与远端回读全部通过。
