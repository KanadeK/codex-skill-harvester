...
8. When artifact provenance is an acceptance requirement, download only each expected asset into a temporary directory and verify it against the stated repository:
   ...
   `gh release verify-asset TAG TEMP/ASSET -R OWNER/REPO --format json`
   Record the asset name, exact release tag, verified owner or repository, command result, and GitHub attestation URL. The release-specific verifier must bind the downloaded digest to an attestation associated with that Release; a generic repository attestation is insufficient. Do not run the asset. A valid attestation proves provenance, not safety.
