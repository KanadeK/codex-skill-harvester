...
The active full-campaign branch currently configures 198 revision-pinned executable endpoints and 1,622 unique Domain × Intent queries. The latest reviewable checkpoint has executed 432 queries successfully from the persisted cycle (435 attempts, three recoverable GitHub rate-limit failures, 53 discovery hits, and 1,189 queries still pending). These are measured separately from inventory and queued-work capacity; parent campaign completion still depends on the policy-owned objective or an explicit controller end.
